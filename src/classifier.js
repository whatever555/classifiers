...
Classifier.prototype.categorize = function (text) {
  ...
  //Find the groups
  for (var k in this.dictionaries) {
    groups.push(k);
  }
  ...
  for (var i = 0; i < words.length; i++) {
    ...
    for (var j = 0; j < groups.length; j++) {
      ...
      probs[groups[j]] = this.dictionaries[groups[j]][words[i]]
        ? this.dictionaries[groups[j]][words[i]]
        : 0;
    }
    ...
    for (var j = 0; j < groups.length; j++) {
      ...
      !probs[groups[j]]
        ? (probabilities[words[i]][groups[j]] = 0)
        : (probabilities[words[i]][groups[j]] =
            probs[groups[j]] / sums[words[i]]);
    }
